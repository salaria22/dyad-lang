...
export class FSHandler implements DAEHandler {
  ...
  private async createDirectory(dir: string): Promise<void> {
    ...
    try {
      ...
      const dirExists = await this.fs
        ...
        .catch((e) => {
          console.error(`Error while checking if directory ${dir} exists: `, e);
          fslog("    Error!!!");
        });
...
`;
      ...
      console.error(`Error while writing file ${key.name}: `, e);
      fslog("...exception thrown");
      throw e;
    }
  }
...
`
      ...
      console.error(
        `Error while closing module ${module.join(
          "."
